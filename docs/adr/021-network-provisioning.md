...
- The companion service is granted access to the full `org.freedesktop.NetworkManager.*` polkit namespace. An initial implementation restricted the rule to three specific action IDs (`network-control`, `wifi.scan`, `wifi.share.open`), but live hardware testing showed that `nmcli connection add` triggers a different action ID (`settings.modify.system` or similar) not covered by that list, causing "Insufficient privileges" failures. More broadly, the exact action IDs NM checks vary across NM versions and operation types — maintaining a narrow allowlist for a dedicated no-login system account is fragile with no meaningful security benefit. The `companion` user has no shell, no home directory, and no other purpose.
