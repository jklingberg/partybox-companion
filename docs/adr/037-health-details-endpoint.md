...
The new `companionHealthItem()` reads `S.healthDetails.tasks` (fetched alongside the other Promise.all calls in `refresh()`, with `withAuth: true`) and surfaces the first task that is currently backed off after a crash — `state === 'waiting' && total_failures > 0` — as `"<Humanized task name> is restarting (<N> failures)"`. Task names are humanized generically (`kebab-case` → `Title case prose`, with a small `TASK_NAME_ACRONYMS` table for exceptions like `ble` → `BLE`) rather than hardcoded per name, so a task added to `__main__.py`'s `supervisor.register()` calls in the future shows up sensibly without a frontend change. No recovery-action button is offered on this row: unlike Spotify's "Restart" action, the Supervisor already restarts the task automatically with backoff — there's nothing for the user to click that the appliance isn't already doing.

## Explicitly out of scope for this ADR

Raised in review; recorded here rather than acted on, since none of them are load-bearing today:

- **This is a single-consumer endpoint, not a general diagnostics API.** The response shape mirrors `TaskHealth` closely on purpose — there is exactly one consumer (the Portal, built and deployed atomically with this endpoint in the same package), so a stable, versioned, decoupled-from-implementation contract would be speculative design for a client that doesn't exist yet. If a second consumer shows up (an external monitoring tool, a CLI), that's the point to introduce a real contract — the project already has a rule of thumb for this exact situation (ADR-032: generalize at the third or fourth instance, not before).
- **The frontend re-derives "is this task down" from `state`/`total_failures` rather than the backend exposing a `healthy: bool`.** Same reasoning: one JS expression, one consumer, well-commented at its one call site. Collapsing to a boolean would also throw away information a future second consumer might want (e.g. an ops view distinguishing "never failed" from "failed once, recovered"). Worth revisiting if/when a second consumer needs the derived signal instead of the raw fields.
- **`last_exception` stays a formatted string, not a structured field.** Documented on `TaskHealthResponse` as informational-only, not machine-readable, and not a format callers should parse — so changing what goes into that string later (e.g. truncating, redacting a path) isn't an API break as long as it stays a string meant for display.
