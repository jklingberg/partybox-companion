...
**`beacon_seen` is scan-driven and eventually consistent, not a live signal.** It records what the *most recent* reconnect-loop scan saw, and is only refreshed when the next scan runs. Two bounded staleness windows follow: a speaker that powers off right after a scan is reported `"unreachable"` (rather than `"off"`) until the next scan completes, and vice versa. This is deliberate — presence rides along on the scan `DeviceManager` already runs, and a dedicated always-on observing scan would reintroduce exactly the scan/connect radio contention that caused the 2026-07-17 audio-stutter incident. Consumers must treat `"unreachable"` vs `"off"` as an eventually-consistent hint (scan-cadence granularity, roughly a minute), not a real-time truth — which is also why the idle-shutdown timer above judges both against generous thresholds rather than acting quickly on either.
...
## Follow-up: throttle the liveness probe once standby is confirmed (2026-09-14)

The original "Consequences" section above accepted, as a known trade-off, that "every health-check cycle now performs a real vendor-protocol round trip even when nothing has changed, adding minor BLE control traffic every 15 s for the life of the connection." Reviewing appliance logs from a speaker left in standby for multiple days quantified that: `_poll_liveness()` had logged its "marking standby" line — and, more relevantly, attempted the underlying battery/firmware GATT round-trip — roughly every 21 s continuously for two days (~7,850 probe cycles), with no backoff once standby was already confirmed. Each miss costs a real ATT write and the speaker's BLE radio waking to (not) answer it, for a state that's already known.

**Revised decision:** `DeviceManager._drain_with_health_check()` keeps `verify_connection()` on the unchanged `_HEALTH_CHECK_INTERVAL` cadence (15 s) — that call is what catches a dead link promptly (ADR-040) and stays cheap since it never reaches the speaker's control-plane logic when the link is already down. `_poll_liveness()` itself, however, is now only invoked every cycle while the speaker's awake/asleep state is still unconfirmed (`_liveness_misses < _LIVENESS_MISS_LIMIT`); once standby is confirmed, it backs off to `_STANDBY_LIVENESS_POLL_INTERVAL` (120 s) — the same relaxed-cadence pattern this decision's "Existing signals" already used for `_STREAMING_HEALTH_CHECK_INTERVAL` (ADR precedent, opposite end of the awake/asleep spectrum: a speaker that's audibly playing has nothing new to report every 15 s either).

**Accepted trade-off:** the up-to ~30–45 s standby-detection latency noted above is unchanged (that transition still probes every cycle), but the *symmetric* on/off transition — the speaker waking back up while the daemon still believes it's asleep — now has its own latency, bounded by `_STANDBY_LIVENESS_POLL_INTERVAL`: up to ~120 s before the daemon notices and flips `speaker_state` back to `"on"`, which in turn is what unblocks `AudioService`'s A2DP reconnect. 120 s was chosen as a middle ground — enough to cut the standby-state battery/radio cost by roughly 8x, short enough that a user turning the speaker back on doesn't wait multiple minutes for the Portal and audio to catch up. A flat 5 min interval was considered and rejected for exactly this reason: it would multiply this same latency proportionally, and — had it applied to `verify_connection()` too rather than being scoped to `_poll_liveness()` alone — would also have degraded ADR-040's dead-link detection, which is why the two probes were split onto independent cadences rather than uniformly slowing the whole health-check loop.

Related: [ADR-040](040-health-check-probe-tolerance.md) (the `verify_connection()` cadence this follow-up deliberately leaves untouched).
