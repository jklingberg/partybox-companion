...
### WirePlumber/PipeWire version skew — root cause and fix (2026-07-02)

A second, distinct flapping failure mode was root-caused on RC12 (Pi 3B+, PartyBox 520). It produces a superficially similar symptom (`audio_ready` never becomes `true`, `bluetoothd`'s log shows every A2DP endpoint repeatedly registering and unregistering) but has a different signature and a different cause from the "rapid-cycling" mode above: endpoints flapped in bursts roughly **once per second** (not once per ~7 s), *all* endpoints churned together under a brand-new D-Bus connection each cycle (not a single `MediaTransport1` disappearing), and it occurred even with `vcgencmd get_throttled` reading `0x0` and zero `Unexpected continuation frame` kernel errors — ruling out the BCM4345 HCI UART corruption this ADR previously implicated.

**Root cause, confirmed via live `busctl monitor --system org.bluez` capture and `WIREPLUMBER_DEBUG=3`:**

1. RC12 shipped `wireplumber 0.4.13` (Debian bookworm) running against `pipewire 1.2.7` (Raspberry Pi's own apt repo, which overrides Debian's `pipewire 0.3.65` — already required for camera/HAT support). WirePlumber 0.4.x targets PipeWire 0.3.x internals; this pairing is unsupported upstream.
2. Upgrading to `wireplumber 0.5.8` (from `bookworm-backports`, the version meant to pair with PipeWire 1.x) did not by itself fix the flap — it changed the symptom to *no* endpoints ever registering. Reading `/usr/share/wireplumber/scripts/monitors/bluez.lua` found the actual mechanism: the bluez5 SPA device monitor is only created while `logind` reports the seat state as `"active"` (`startStopMonitor()`, gated by `config.seat_monitoring = Core.test_feature("monitor.bluez.seat-monitoring")`). On this headless appliance, logind's seat state cycles between `"online"` and `"closing"` and **never** reaches `"active"`. Under 0.4.13 the equivalent seat-driven logic was what repeatedly created and destroyed the whole bluez5 monitor (and therefore every endpoint) roughly once a second — this is the actual mechanism behind the flap, not HCI-level corruption.
3. `wireplumber.conf` ships a purpose-built profile for exactly this scenario: `main-embedded` ("Typical profile for embedded use cases, systemwide without maintaining state"), which inherits `mixin.systemwide-session` (`support.logind = disabled`, `monitor.bluez.seat-monitoring = disabled`, plus other desktop-only features) and `mixin.stateless`. Running WirePlumber with `-p main-embedded` (via a systemd user-service drop-in) stopped the flap entirely — confirmed stable (zero endpoint churn) over multiple minutes, versus dozens of churn cycles per minute before.
...
- `wireplumber` is installed from `bookworm-backports`, pinned to `0.5.8-1~bpo12+1` (not Debian bookworm's `0.4.13-1`).
- `image/config/wireplumber-embedded-profile.conf` is deployed as a `wireplumber.service` (user-session) systemd drop-in, running `wireplumber -p main-embedded`.
...
**Not yet resolved:** with the flap fixed, live testing surfaced `br-connection-unknown` from `ConnectProfile`. This is plausibly *not* a new bug — it may be the stale-`~/.local/state/wireplumber/`-after-restart issue this ADR's `install.sh` step 10c(5) already documents and works around via a boot-time `tmpfiles.d` clear. All testing in this session used `systemctl --user restart wireplumber` / `systemctl restart companion` (service-level restarts), never a full `reboot`, so that tmpfiles.d clear never fired. A full reboot should be the first thing tried before treating `br-connection-unknown` as a new investigation.
