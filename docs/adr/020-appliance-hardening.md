...
- The image ships with a default `pi` user in the `sudo` group and `openssh-server` installed. **Update:** the shared `pi`/`raspberry` password and `PasswordAuthentication yes` described when this ADR was written have been removed — see [ADR-042](042-ssh-hardening.md), which re-evaluated this exactly as flagged here before v1.0. SSH now ships disabled by default, is enabled and key-provisioned entirely through the Portal, and `pi`'s console password is randomised per device on first real boot rather than fixed at build time.
