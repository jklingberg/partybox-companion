...
**SSH access:** `ssh pi@partybox.local` (mDNS, preferred) or `ssh pi@partybox` (router DNS), once enabled and key-provisioned via the Portal (Settings → SSH access) — SSH ships disabled by default with no fixed password on every image ([ADR-043](adr/043-ssh-hardening.md)). See [CLAUDE.md](../CLAUDE.md) for the one-time enable flow.
