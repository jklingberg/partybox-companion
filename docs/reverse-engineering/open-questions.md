...
- How is the speaker woken from standby? It stops accepting new BLE bonds in standby; does it still accept a control connection from an already-bonded host to receive the power-on frame, or is a separate wake path needed? **Partially narrowed by static analysis (2026-07-18)** — see [discoveries.md](discoveries.md#likely): the app's `ReqCommandProtocol` base interface has an `isDefibrillation` flag (strong naming evidence for a dedicated wake command, distinct from the ordinary command set) and its advertisement parser is designed around explicit `Ble_Standby_Info`/`BT_Connection_Info` fields — but the concrete command class, its opcode, and which BLE address it targets were not recoverable from this build's decompile (R8-optimized past the point of text-based recovery). **The manufacturer-specific-AD hypothesis is now closed** — a live 10-minute passive scan on the appliance found zero advertisements with non-empty `manufacturer_data` (see [protocol.md](protocol.md#offsets-02-and-a-manufacturer-specific-ad-hypothesis-closed-2026-07-18)); those fields most likely live in still-unexplained offsets of the FDDF payload this project already reads, not a separate structure. Still open: an on-wire HCI capture of the JBL app performing the wake, and a passive FDDF capture spanning a `"standby"`→`"on"` transition (today's capture happened to land entirely in `"unreachable"`) to diff against the always-zero-so-far offsets and isolate `Ble_Standby_Info`.
- **Two LE identities both advertise `"JBL PartyBox 520"` (observed 2026-07-18, not investigated).** During the passive scan above, a second rotating address carried completely different service data — UUID `0000fe2c` (2 zero bytes) plus an empty `00001853` (Bluetooth SIG "Public Broadcast Announcement" / LE Audio Auracast) — alongside the already-documented FDDF-carrying address. Possibly a second, Auracast-related advertising set from the same physical speaker (the PartyBox 520 markets LE Audio/Auracast support). Unclear if relevant to anything this project does; noted in case it resurfaces.
